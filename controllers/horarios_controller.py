    ...
    if request.method == 'POST':
        try:
            ...
            if not fecha_str or not proyecto_id or not personal_ids:
                flash("Debes completar la fecha, seleccionar un proyecto y al menos un trabajador.", "warning")
            ...
            # Lista para llevar seguimiento de nuevos proyectos asignados
            nuevos_proyectos_agregados = 0 
            ...
            for p_id_str in personal_ids:
                ...
                # ------------------------------------------------------------------
                # 📢 NUEVA LÓGICA DE SINCRONIZACIÓN (ProyectoPersonal)
                # ------------------------------------------------------------------
                
                # 1. Verificar si el trabajador ya tiene asignación maestra para este proyecto
                existe_asignacion_maestra = ProyectoPersonal.query.filter_by(
                    personal_id=p_id_int, 
                    proyecto_id=proyecto_id
                ).first()
                
                # 2. Si NO existe la asignación maestra, la creamos
                if not existe_asignacion_maestra:
                    nueva_maestra = ProyectoPersonal(
                        personal_id=p_id_int,
                        proyecto_id=proyecto_id
                        # Puedes inicializar otros campos de ProyectoPersonal si tu modelo los tiene
                    )
                    db.session.add(nueva_maestra)
                    nuevos_proyectos_agregados += 1
                
                # ------------------------------------------------------------------
                # FIN LÓGICA DE SINCRONIZACIÓN
                # ------------------------------------------------------------------

                # Crear la nueva asignación diaria
            ...
            flash_message = f" {len(personal_ids)} trabajadores asignados al proyecto '{proyecto.nombre}' para el {fecha.strftime('%d/%m/%Y')}."
            if nuevos_proyectos_agregados > 0:
                 flash_message += f" (Se agregaron {nuevos_proyectos_agregados} nuevas asignaciones maestras a proyectos)."
...
def eliminar_asignacion_diaria(id):
    ...
    try:
        ...
        db.session.commit()
        flash(f"Asignación de {personal_nombre} al proyecto {proyecto_nombre} para el {fecha.strftime('%d/%m/%Y')} eliminada.", "success")
    except Exception as e:
        db.session.rollback()
